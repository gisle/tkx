...
C<Tkx> is a Perl module that makes the I<Tk> toolkit available to Perl
...
In line 9 we initialize $IS_AQUA constant.  Aqua is the native
...
In line 17 we ask I<Tk> to start processing events, by invoking
...
and C<show_manual> functions before the appliation ships. :-)
...
to use an explict call to set up this binding as done in line 38.
...
special significance to I<Tk>.  Menu items added to the "apple" menu
...
the menu just right of the apple.  If not provided C<Tk> will provide
its own "About" entry that will tell you about what version of
I<Tcl/Tk> you are using.  A menu called "help" menu will be flushed
right on Unix even though this style seems to be out of fashion in
modern Unix applications.

The Tkx distribution contain a script called C<menu> which is a
runnable version of the program shown here.  You might want to use
this as a starting point for your own Tkx application.

=head2 Using Tcl packages

The C<Tkx> Perl module binds to the I<Tk> toolkit through I<Tcl>,
which allows for C<Tkx> to make use of any available I<Tcl> packages.
You can load these packages with C<Tkx::package_require>:
